		...
		for(int i = 0; i < arr.length; i++) {
			for(int j = 0; j < arr.length; j++) {
				...
				for(int k = 0; k < 4; k++) {
					...
					//int around_num = arr[r_index][c_index]; 아래 범위 조건 줘놓고 이래 버리니까 오류가 나징..
					...
					if(r_index >= 0 && r_index < arr[i].length && c_index >= 0 && c_index < arr[i].length && arr[r_index][c_index] >= arr[i][j]) {
						flag = false;
						break;
					}
				}
